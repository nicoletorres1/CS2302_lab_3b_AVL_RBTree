...
def print_anagrams(word, prefix=""): # this method is given by lab
    if len(word) <= 1:
        str = prefix + word
...
def buildTree(response, filename):
    """
...
if __name__ == '__main__':
    while True:
        try:
            response = input("What type of tree would you like to use? Enter 1 for AVL tree or 2 for Red-Black Tree:\n")
            filename = input("Enter the name of the file to use for the vocabulary: ")
            if buildTree(response, filename):
                break
        except:
            print("Invalid file!")
